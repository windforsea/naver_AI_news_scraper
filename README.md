...
- **모던 3열 글래스모피즘 워크스페이스**:
  ...
  - **2열(중앙)**: gpt-5.6-luna 마크다운 보고서 뷰어 (`기본 3대 브리핑` vs `맞춤 테마 브리핑` 모드 뱃지 실시간 토글, 이전 보고서 이력 선택기)
  - **3열(우측)**: AI 브리핑 비서 대화창 & 활성 지시사항 칩 관리 바

### 7. 📂 보고서 생명주기 관리 & 이전 보고서 이력 조회 (History Tracking)
- **앱 시작 시 자동 복원**: 앱 실행 시 SQLite DB 및 `reports/` 폴더를 조회하여 가장 최근에 생성된 보고서 본문과 당시 수집된 기사 목록을 뷰어와 테이블에 자동 복원합니다.
- **`[📂 이전 보고서 목록]` 드롭다운 셀렉터**: 중앙 헤더에 과거 보고서 목록(생성 일시 및 제목)을 제공하여 언제든 원하는 과거 보고서와 매핑된 원문 기사 리스트를 원클릭으로 전환하여 열람할 수 있습니다.
- **스마트 파일 열기 (`open_report_file`)**: `📄 열기` 버튼 클릭 시 현재 활성 보고서뿐 아니라, 메모리가 비어있더라도 최신 `.md` 파일을 탐색하여 윈도우 기본 마크다운/텍스트 뷰어로 즉시 실행합니다.
