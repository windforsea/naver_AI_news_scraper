...
### 5. 🧑‍💼 AI 브리핑 어시스턴트 챗봇 (Human-in-the-loop 맞춤 지시 반영)
- 사용자가 실시간 챗봇에게 *"이번 보고서는 생성형 AI 보안 위협을 최우선으로 다뤄줘"*, *"스타트업 투자 관점에서 시사점을 써줘"* 등의 맞춤형 요구사항을 전달할 수 있습니다.
- 상단의 **[수집 & AI 보고서 생성]** 실행 시, 수집된 기사 원문과 함께 **사용자의 챗봇 지시사항(`chat_history`)을 프롬프트에 동시 주입**하여 100% 사용자 의도가 반영된 맞춤형 전문 브리핑을 완성합니다.

### 6. 🖥️ 모던 3열 데스크톱 대시보드 (pywebview + Flatpickr)
- 외부 브라우저 없이 독립 실행되는 경량 데스크톱 윈도우 UI를 구축했습니다.
- **1열(좌측)**: 수집 기사 목록 및 CSV/DB 데이터 폴더 열기
- **2열(중앙)**: gpt-5.6-luna 마크다운 보고서 실시간 렌더링 및 클립보드 복사
- **3열(우측)**: AI 브리핑 비서 챗봇 사이드바 (실시간 지시사항 대화창)
- Flatpickr 기반 단일일/기간 범위(Range) 선택 및 DB 누적 통계 실시간 배지 제공.
